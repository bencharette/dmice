...
For each simulated event:
  ...
  2. Get IC-only LineFit from L2 processing
  3. Compute IC+DM-Ice LineFit by appending a real DM-Ice hit (MC truth transit time) to IC pulses
  4. Compare angular errors of both reconstructions vs MC truth

Run on NPX inside IceTray environment:
    /cvmfs/icecube.opensciencegrid.org/py3-v4.3.0/RHEL_9_x86_64/metaprojects/icetray/v1.12.1/env-shell.sh \
        python sim_linefit_comparison.py -i muon_l2.i3 -g gcdfile.i3.gz [--max-events N]
"""
...
# LineFit keys to try
LINEFIT_KEYS = ["LineFit", "PoleMuonLinefit", "MPEFit", "SPEFit2",
                "PoleMuonLlhFit"]
...
def main():
    ...
    parser.add_argument('-i', '--input', required=True, help='Input L2 i3 file')
    parser.add_argument('-g', '--gcd', required=True, help='GCD file')
    ...
    parser.add_argument('--plot', default=None, metavar='OUTPUT.png',
                        help='Save two-histogram angular resolution plot to this file')
    ...
    while f.more():
        ...
        # Closest approach to each DM-Ice detector
        ...
        if ca_min < 500:  # within 500m of DM-Ice
            n_near_dmice += 1
        ...
        std_reco_key = None
        std_speed = np.nan
        std_zenith = std_azimuth = np.nan
        for key in LINEFIT_KEYS:
            if key in frame:
                lf = frame[key]
                std_dir = np.array([lf.dir.x, lf.dir.y, lf.dir.z])
                std_zenith = np.degrees(lf.dir.zenith)
                std_azimuth = np.degrees(lf.dir.azimuth)
                std_reco_key = key
                std_speed = lf.speed
                break

        # If no stored LineFit, compute it from pulses
        x, y, z, t_abs, w, n_doms = extract_hits(frame, dom_pos)
        if std_dir is None and x is not None and n_doms >= 4:
            lf = run_ic_linefit(x, y, z, t_abs, w)
            if lf is not None:
                std_dir = np.array([lf['dx'], lf['dy'], lf['dz']])
                std_zenith = lf['zenith_deg']
                std_azimuth = lf['azimuth_deg']
                std_reco_key = 'computed_linefit'
                std_speed = lf['speed_m_ns']

        if std_dir is None:
            continue
        n_with_linefit += 1

        ic_ang_err = angular_diff_deg(mc_dir, std_dir)
        dm_pos = DMICE_POS[closest_det]

        # Also compute analytic IC-only LineFit for apples-to-apples comparison
        ic_analytic_ang_err = np.nan
        if x is not None and n_doms >= 4:
            lf_ic = run_ic_linefit(x, y, z, t_abs, w)
            if lf_ic is not None:
                ic_analytic_dir = np.array([lf_ic['dx'], lf_ic['dy'], lf_ic['dz']])
                ic_analytic_ang_err = angular_diff_deg(mc_dir, ic_analytic_dir)
        ...
        if x is not None and n_doms >= 4 and std_speed > 0.1 and muon is not None:
            mc_dir = np.array([muon.dir.x, muon.dir.y, muon.dir.z])
            ...
            if cfit is not None:
                ...
                cfit_ang_diff = angular_diff_deg(std_dir, cdir)
        ...
        results.append(dict(
            ...
            std_reco_key=std_reco_key,
            std_zenith_deg=std_zenith,
            std_azimuth_deg=std_azimuth,
            std_speed_m_ns=std_speed,
            ic_only_ang_err_deg=ic_ang_err,
            ic_analytic_ang_err_deg=ic_analytic_ang_err,
            ...
            n_doms=n_doms if x is not None else 0,
        ))
    ...
    # Summary
    print("\n── Reco keys found ─────────────────────────────────────")
    print(df['std_reco_key'].value_counts().to_string())

    print("\n── IC-only LineFit angular error vs MC truth (deg) ─────")
    print(df['ic_only_ang_err_deg'].describe().to_string())

    print("\n── Analytic IC-only LineFit angular error vs MC truth (deg) ──")
    print(df['ic_analytic_ang_err_deg'].describe().to_string())

    has_cfit = df['cfit_ang_err_deg'].notna()
    if has_cfit.any():
        print("\n── IC+DM-Ice LineFit angular error vs MC truth (deg) ──")
        print(df.loc[has_cfit, 'cfit_ang_err_deg'].describe().to_string())

        print("\n── Angular shift from adding DM-Ice (deg) ──────────────")
        print(df.loc[has_cfit, 'cfit_ang_diff_deg'].describe().to_string())

        # Compare: does DM-Ice improve over analytic IC-only? (apples-to-apples)
        has_both = has_cfit & df['ic_analytic_ang_err_deg'].notna()
        improved_vs_analytic = df.loc[has_both, 'cfit_ang_err_deg'] < df.loc[has_both, 'ic_analytic_ang_err_deg']
        improved_vs_stored   = df.loc[has_cfit, 'cfit_ang_err_deg'] < df.loc[has_cfit, 'ic_only_ang_err_deg']
        print("\n── DM-Ice improves direction? ──────────────────────────")
        print("  vs stored PoleMuonLinefit:  {} / {} ({:.1f}%)".format(
            improved_vs_stored.sum(), has_cfit.sum(), 100 * improved_vs_stored.mean()))
        print("  vs analytic IC LineFit:     {} / {} ({:.1f}%)".format(
            improved_vs_analytic.sum(), has_both.sum(), 100 * improved_vs_analytic.mean()))
        print("  Stored PoleMuonLinefit median:       {:.2f} deg".format(
            df.loc[has_cfit, 'ic_only_ang_err_deg'].median()))
        print("  Analytic IC-only median:             {:.2f} deg".format(
    ...
    for ca_bin, grp in df.groupby('ca_bin', observed=True):
        ...
        if has.any():
            print("  CA {:>15s}:  n={:3d}  IC={:.1f}°  IC+DM={:.1f}°  shift={:.1f}°".format(
                str(ca_bin), has.sum(),
                grp.loc[has, 'ic_only_ang_err_deg'].median(),
    ...
    if args.plot:
        ic_err        = df['ic_only_ang_err_deg'].dropna()
        ic_analytic   = df['ic_analytic_ang_err_deg'].dropna()
        ...
        max_err = max(ic_err.max() if len(ic_err) else 0,
                      ic_analytic.max() if len(ic_analytic) else 0,
        ...
        if len(ic_analytic) > 0:
            ax.hist(ic_analytic, bins=bins, histtype='step', linewidth=2,
                    color='green',
                    label='Analytic IC LineFit  median={:.1f}°'.format(ic_analytic.median()))
            ax.axvline(ic_analytic.median(), color='darkgreen', linewidth=2, linestyle=':')
